...
- (void)retrieveMessageArchiveWithFields:(NSArray *)fields withResultSet:(XMPPResultSet *)resultSet queryId: (NSString *) queryId {
	[self retrieveMessageArchiveAt:nil withFields:fields withResultSet:resultSet queryId:queryId];
}

- (void)retrieveMessageArchiveAt:(XMPPJID *)archiveJID withFields:(NSArray *)fields withResultSet:(XMPPResultSet *)resultSet queryId: (NSString *) queryId {
    ...
    [self retrieveMessageArchiveAt:archiveJID withFormElement:formElement resultSet:resultSet queryId:queryId];
}
...
- (void)retrieveMessageArchiveAt:(XMPPJID *)archiveJID withFormElement:(NSXMLElement *)formElement resultSet:(XMPPResultSet *)resultSet queryId: (NSString *) queryId {
	[self performBlockAsync:^{
        ...
        NSString * theQueryId = queryId ? queryId : XMPPStream.generateUUID;
        
		[self->_outstandingQueryIds addObject:theQueryId];
		
		NSXMLElement *queryElement = [NSXMLElement elementWithName:@"query" xmlns:XMLNS_XMPP_MAM];
        [queryElement addAttributeWithName:QueryIdAttributeName stringValue:theQueryId];
...
- (void)handleMessageArchiveIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)trackerInfo {
	...
	if ([[iq type] isEqualToString:@"result"]) {
        ...
        [self retrieveMessageArchiveAt:originalArchiveJID withFormElement:originalFormElement resultSet:pagingResultSet queryId:nil];
	} else {
        NSString * queryId = [[trackerInfo.element elementForName:@"query" xmlns:XMLNS_XMPP_MAM] attributeForName:@"queryid"].stringValue;
        [multicastDelegate xmppMessageArchiveManagement:self didFailToReceiveMessages:iq queryId: queryId];
	}
}
